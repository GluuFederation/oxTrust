...
public class PatchUtils implements Serializable {
	...
	public GluuCustomPerson replacePatch(User source, GluuCustomPerson destination) throws Exception {
					...
					if (email != null && email.getType() != null) {
						for (Email newEmail : newemails) {
							if ((newEmail.getType() != null)
									&& newEmail.getType().getValue().equals(email.getType().getValue())) {
								emails.remove(email);
								emails.add(newEmail);
					...
					if (address != null && address.getType() != null) {
						for (Address newAddress : newaddresses) {
							if ((newAddress.getType() != null)
									&& newAddress.getType().getValue().equals(address.getType().getValue())) {
		...
		if (source.getPhoneNumbers() != null && source.getPhoneNumbers().size() > 0) {
			List<PhoneNumber> phoneNumbers = copyUtils2.getAttributeListValue(destination, PhoneNumber.class,
					"oxTrustPhoneValue");
				...
				for (PhoneNumber phoneNumber : phoneNumbers) {
					if (phoneNumber != null && phoneNumber.getType() != null) {
						for (PhoneNumber newPhoneNumber : newPhoneNumbers) {
							if ((newPhoneNumber.getType() != null) && (phoneNumber.getType().getValue() != null)
									&& newPhoneNumber.getType().getValue().equals(phoneNumber.getType().getValue())) {
				...
				for (Photo photo : photos) {
					if (photo != null && photo.getType() != null) {
						for (Photo newPhoto : newPhotos) {
							if (newPhoto.getType() != null
									&& newPhoto.getType().getValue().equals(photo.getType().getValue())) {
								photos.remove(photo);
								photos.add(newPhoto);
				...
				for (Role role : roles) {
					if (role != null && role.getType() != null) {
						for (Role newRole : newRoles) {
							if ((newRole.getType() != null)
									&& newRole.getType().getValue().equals(role.getType().getValue())) {
								roles.remove(role);
								roles.add(newRole);
		...
		if (source.getEntitlements() != null && source.getEntitlements().size() > 0) {
			List<Entitlement> entitlements = copyUtils2.getAttributeListValue(destination, Entitlement.class,
					"oxTrustEntitlements");
				...
				for (Entitlement entitlement : entitlements) {
					if (entitlement != null && entitlement.getType() != null) {
						for (Entitlement newEntitlement : newEentitlements) {
							if ((newEntitlement.getType() != null)
									&& newEntitlement.getType().getValue().equals(entitlement.getType().getValue())) {
		...
		if (source.getX509Certificates() != null && source.getX509Certificates().size() > 0) {
			List<X509Certificate> X509Certificates = copyUtils2.getAttributeListValue(destination,
					X509Certificate.class, "oxTrustx509Certificate");
				...
				for (X509Certificate X509Certificate : X509Certificates) {
					if (X509Certificate != null && X509Certificate.getType() != null) {
						for (X509Certificate newX509Certificate : newX509Certificates) {
							if ((newX509Certificate.getType() != null) && newX509Certificate.getType().getValue()
									.equals(X509Certificate.getType().getValue())) {
				...
				while (addressesIt.hasNext()) {
					...
					if (address != null && address.getType() != null) {
						while (newaddressesIt.hasNext()) {
							...
							if (newaddress.getType().getValue() != null
									&& newaddress.getType().getValue().equals(address.getType().getValue())) {
								addressesIt.remove();
							}
						}
			...
			if (phoneNumbers != null && phoneNumbers.size() > 0) {
				...
				while (phoneNumbersIt.hasNext()) {
					...
					while (newPhoneNumbersIt.hasNext()) {
						...
						if (newPhoneNumber.getType() != null
								&& newPhoneNumber.getType().getValue().equals(phoneNumber.getType().getValue())) {
					...
					if (old != null && old.getType() != null) {
						while (newphotosIt.hasNext()) {
							...
							if (newelement.getType() != null
									&& newelement.getType().getValue().equals(old.getType().getValue())) {
								photosIt.remove();
							}
						}
					}
