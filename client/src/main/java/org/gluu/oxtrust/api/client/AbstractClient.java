...
public class AbstractClient<T> {
	 ...
	 * 
	 * @param id
	 *            ID of the entity (inum bu default)
	...
	public T read(String id) throws ClientErrorException {
		...
		return resource.request(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).get(entityClass);
	}
	...
	public String create(T requestEntity) throws OxTrustAPIException {
		Response response = webTarget.path("create").request()
